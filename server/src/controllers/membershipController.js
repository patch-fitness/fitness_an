...
const buildUpdateQuery = (payload) => {
  const allowedFields = ['title', 'name', 'price', 'package_type', 'duration_in_months', 'gymId', 'trainer_id', 'schedule'];
...
exports.getMemberships = asyncHandler(async (req, res) => {
  ...
  try {
    // Try to select with trainer_id and schedule (if columns exist)
...
exports.createMembership = asyncHandler(async (req, res) => {
  ...
  // Check if trainer_id and schedule columns exist by trying to insert
  try {
    const [result] = await pool.execute(
      `INSERT INTO Memberships (title, name, price, package_type, duration_in_months, gymId, trainer_id, schedule)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [finalTitle, finalName, price, package_type, finalDuration, gymId, trainer_id || null, schedule || null]
    );
