...
const selectableColumns = ['id', 'name', 'mobileNo', 'sex', 'degree', 'gymId'];

const buildUpdateQuery = (payload) => {
  const allowedFields = ['name', 'mobileNo', 'sex', 'degree', 'gymId'];
...
exports.createTrainer = asyncHandler(async (req, res) => {
  ...
  const { name, mobileNo, sex = null, degree = null, gymId } = req.body;

  const [result] = await pool.execute(
    `INSERT INTO Trainers (name, mobileNo, sex, degree, gymId)
     VALUES (?, ?, ?, ?, ?)`,
    [name, mobileNo, sex, degree, gymId]
  );
